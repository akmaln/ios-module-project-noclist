...
let agentA = (coverName: "Ethan Hunt", realName: "Tom Cruise", accessLevel: 8, compramised: false)
let agentB = (coverName: "Jim Phelps", realName: "Jon Voight", accessLevel: 9, compramised: true)
let agentC = (coverName: "Claire Phelps", realName: "Emmanuelle Beart", accessLevel: 5, compramised: false)
let agentD = (coverName: "Eugene Kittridge", realName: "Henry Czerny", accessLevel: 10, compramised: true)
let agentE = (coverName: "Franz Krieger", realName: "Jean Reno", accessLevel: 4, compramised: false)
let agentF = (coverName: "Luther Stickell", realName: "Ving Rhames", accessLevel: 4, compramised: false)
let agentG = (coverName: "Sarah Davies", realName: "Kristin Scott Thomas", accessLevel: 5, compramised: true)
let agentH = (coverName: "Max RotGrab", realName: "Vanessa Redgrave", accessLevel: 4, compramised: false)
let agentI = (coverName: "Hannah Williams", realName: "Ingeborga Dapkunaite", accessLevel: 5, compramised: true)
let agentJ = (coverName: "Jack Harmon", realName: "Emilio Estevez", accessLevel: 6, compramised: true)
let agentK = (coverName: "Frank Barnes", realName: "Dale Dye", accessLevel: 9, compramised: false)
//: ## Step 2
//: Place the above constants inside an array. Declare this array as a constant as well.
let agents = [agentA, agentB, agentC, agentD, agentE, agentF, agentG, agentH, agentI, agentJ, agentK]
...
func numOfCompAgents() -> Int{
    ...
    var totalCompAgents = 0
    for agent in agents {
        if agent.compramised == true {
            totalCompAgents += 1
        }
    }
    return totalCompAgents
}
...
func findCleanAgents() -> Array<Any> {
    ...
    var uncompAgents: [Any] = []
    
    for agent in agents {
        if agent.compramised == false {
...
func findHighRisk(){
    for agent in agents {
        if agent.accessLevel >= 8 && agent.compramised == false {
            print("\(agent.realName), level: \(agent.accessLevel)")
        } else if agent.accessLevel >= 8 && agent.compramised == true {
            print("\(agent.realName), level: \(agent.accessLevel) **WARNING** **COMPRAMISED**")
...
func agentLevels() {
    ...
    print("\(lowLvl) low level agents, \(midLvl) mid level agents, \(highLvl) high level agents")
}
